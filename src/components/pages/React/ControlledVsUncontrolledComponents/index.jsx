...
function UncontrolledComponent() {
 const inputRef = useRef(null); // etapa 1: iniciar a constante com o valor do useRef
 ...
 return (
  <form onSubmit={(e) => submitUncontrolledForm(e)}>
   <input
    ...
    ref={inputRef} // etapa 2: cria a relação entre a constante com useRef
    required
   />
...
function ControlledComponent() {
 ...
 function submitControlledForm(event) {
  event.preventDefault();
  window.alert(`Você cadastrou a turma "controlada": ${valorDoInput}`);
 }
 ...
 return (
  <form onSubmit={(e) => submitControlledForm(e)}>
   <input
    ...
    value={valorDoInput}
    onChange={(event) => setValorDoInput(event.target.value)}
    required
   />
   <button type="submit">Enviar</button>
  </form>
